...
class Network:
    ...
    def __init__(self, input_layer_size, name="L"):
        ...
        layer_number = 0
        neurons = [
            self.prefix + str(0) + "_" + str(i + 1) for i in range(input_layer_size)
        ]
        ...
        self.layers = {
            layer_name: {
                ...
                "biases_trainable": biases_trainable,
            }
        }
    ...
    def summary(self):
        ...
        for key, layer in self.layers.items():
            ...
            if activation == "":
                activation = None
            print(
                f"Layer: {key}, Neurons: {len(layer['neurons'])}, Activation: {activation}, Weights: {layer['weights'].shape}, Biases: {layer['biases'].shape}"
            )
    ...
    def predict(self, X):
        ...
        assert X.shape == (
            input_dimension,
        ), f"The data point have shape ({input_dimension},)."
    ...
    def forward(self, X):
        ...
        ), f"The data should be {input_dimension}-dimensional, you provided {X.shape[1]}-dim data"
    ...
    def backward(self, X, Y):
        ...
        for layer_number in range(len(self.layers) - 1, 0, -1):
            ...
            if layer_number >= 2:
                ...
                activation_derivative = self.layers[previous_layer_name][
    ...
    def binary_crossentropy_loss_derivative(self, X, Y):
        ...
        return np.add(np.divide(-Y, Y_hat), np.divide(1 - Y, 1 - Y_hat)).reshape(
            Y_hat.shape[0], 1
        )
        ...
        for i in range(num_epochs):
            ...
            if verbose:
                if validation_data == False:
                    ...
                    print(
                        f"loss: {loss:.3f} acc: {acc: 0.0%} val_loss: {val_loss:.3f} val_acc: {val_acc: 0.0%}"
                    )
    ...
    def train_mini_batch(
        ...
        ), "Ensure the batch size is smaller than training set"
            ...
            for index in range(1, len(batches)):
                ...
                for key, layer in self.layers.items():
                    ...
                    if key[-1] == "0":
                        ...
                        # Perform the update
                        self.layers[key]["weights"] = W - np.multiply(
                            learning_rate * dW, self.layers[key]["weights_trainable"]
                        )
                        self.layers[key]["biases"] = B - np.multiply(
                            learning_rate * dB, self.layers[key]["biases_trainable"]
                        )
