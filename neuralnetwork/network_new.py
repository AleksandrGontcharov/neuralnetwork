...
class Network:
    ...
    def fit(self, X_train, Y_train, batch_size, num_epochs, learning_rate, validation_data, verbose):
        ...
        for i in range(num_epochs):
            # Calculate loss
            loss = 1#self.binary_crossentropy_loss(X_train, Y_train)
            acc = self.accuracy(X_train, Y_train)
            for index in tqdm(range(1, len(batches)), desc=f"Epoch {i+1} Loss: {loss} acc: {acc:4f}"):
    ...
    def accuracy(self, X, Y):
        ...
        neuron_outputs = self.forward(X)
        highest_layer_name = self._prefix + str(self.number_of_layers - 1)
        Y_hat = neuron_outputs[highest_layer_name]["Z"]
